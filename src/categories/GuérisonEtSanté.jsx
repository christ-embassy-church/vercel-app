...
import { getCategoryGuerisonEtSante } from "../services";

export default function GuerisonEtSante() {

  function GuerisonEtSanteCard(props) {
  ...
  function ShowCard() {
   ...
   return (
    ...
    const { data, loading, error } = useQuery(getCategoryGuerisonEtSante);
 ...
 return(
      <div>
        <div className="category--page">
            <h1 className="recent--title">Catégorie : Guérison Et Santé</h1>
        </div>
        <div className="alls-list">
            {data.messages.map((message) => {
                return <GuerisonEtSanteCard key={message.id} {...message}/>
            })}    
          </div>
      </div>
 )
