...
import { getCategoryPriere } from "../services";

export default function Priere() {

  function PriereCard(props) {
  ...
  function ShowCard() {
   ...
   return (
    ...
    const { data, loading, error } = useQuery(getCategoryPriere);
 ...
 return(
  <div>
    <div className="category--page">
            <h1 className="recent--title">Catégorie : Prière</h1>
    </div>
    <div className="alls-list">
        {data.messages.map((message) => {
            return <PriereCard key={message.id} {...message}/>
        })}
    </div>
  </div>
 )
