...
import { getCategoryProsperiteEtFinance } from "../services";

export default function ProsperiteEtFinance() {

  function ProsperiteEtFinanceCard(props) {
  ...
  function ShowCard() {
   ...
   return (
    ...
    const { data, loading, error } = useQuery(getCategoryProsperiteEtFinance);
 ...
 return(
      <div>
        <div className="category--page">
            <h1 className="recent--title">Catégorie : Prospérité et Finance</h1>
        </div>
        <div className="alls-list">
            {data.messages.map((message) => {
                return <ProsperiteEtFinanceCard key={message.id} {...message}/>
            })}
        </div>
      </div>
 )
