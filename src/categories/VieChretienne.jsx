...
import { getCategoryVieChretienne } from "../services";


export default function VieChretienne() {
  const categorySlug = useParams()


  function VieChretienneCard(props) {
  ...
  function ShowCard() {
   ...
   return (
    ...
    const { data, loading, error } = useQuery(getCategoryVieChretienne);
 ...
 return(
    <div>
        <div className="category--page">
            <h1 className="recent--title">Catégorie : Vie Chrétienne</h1>
        </div>
        <div className="alls-list">
            {data.messages.map((message) => {
               return <VieChretienneCard key={message.id} {...message}/>
            })}
        </div>
    </div>
 )
