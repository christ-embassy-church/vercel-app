...
import { getCategorySaintEsprit } from "../services";

export default function SaintEsprit() {

  function SaintEspritCard(props) {
  ...
  function ShowCard() {
   ...
   return (
    ...
    const { data, loading, error } = useQuery(getCategorySaintEsprit);
 ...
 return(
      <div>
        <div className="category--page">
            <h1 className="recent--title">Catégorie : Saint-Esprit </h1>
        </div>
        <div className="alls-list">
            {data.messages.map((message) => {
               return <SaintEspritCard key={message.id} {...message}/>
            })}
        </div>
      </div>
 )
